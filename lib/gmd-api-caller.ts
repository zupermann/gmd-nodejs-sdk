...
export class RemoteAPICaller {
    ...
    */
    async apiCall<T = unknown>(method: 'get' | 'post', params: Record<string, string | number | boolean>): Promise<T> {
        const config = { method: method, url: this.baseURL + 'nxt?' + (new URLSearchParams(params as Record<string, string>)).toString() } as AxiosRequestConfig<Record<string, unknown>>;
